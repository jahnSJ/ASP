...
*/
pub fn frames_mag(array:[[Complex<f32>;FRAME_LENGTH];NUMBER_OF_FRAMES]) -> [[f32;FRAME_LENGTH];NUMBER_OF_FRAMES]{
...
*/
pub fn mag(array:[Complex<f32>;FRAME_LENGTH]) -> [f32;FRAME_LENGTH]{
    let mut out= [0.0;FRAME_LENGTH];

    for i in 0..FRAME_LENGTH{
        let temp = array[i].re * array[i].re + array[i].im * array[i].im;
...
pub fn get_frames_with_padding(signal:[f32; BUFFER_SIZE])-> [[f32; FRAME_LENGTH]; NUMBER_OF_FRAMES]{
    ...
    for i in 0..NUMBER_OF_FRAMES{
        for j in 0..ACTUAL_FRAME_LENGTH{
            array_2d[i][j] = signal[j + i*ACTUAL_FRAME_LENGTH];
        }
    }
